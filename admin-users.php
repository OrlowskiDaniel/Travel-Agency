                    ...
                    include("dbcalls/read-users.php");
                        foreach($result as $key => $value) {
                            echo '<div class="admin-users-box box-style-2">';
                                ...
                                echo '</div>';
                                echo '<div class="buttons-users buttons-admin-wrap">';
                                    echo '<button onclick="showEditForm(event)" class="edit-button gray-hover" type="button">Edit</button>';
                                    echo '<div class="form-overlay" style="display: none;">';
                                        echo '<form class="editform" action="dbcalls/update-user.php" method="post">';
                                            echo '<fieldset>';
                                                echo '<label for="username">Username:</label>';
                                                echo '<input type="text" id="username" name="username" placeholder=" ' . $value['username'] .' "value="' . $value['username'] .'" required>';
                                                echo '<label for="email">Email:</label>';
                                                echo '<input type="email" id="email" name="email" placeholder=" ' . $value['email'] .' "value="' . $value['email'] .'" required>';
                                                echo '<label for="phonenumber">Phonenumber:</label>';
                                                echo '<input type="text" id="phonenumber" name="phonenumber" placeholder=" ' . $value['phonenumber'] .' "value="' . $value['phonenumber'] .'" required>';
                                            echo '</fieldset>';
